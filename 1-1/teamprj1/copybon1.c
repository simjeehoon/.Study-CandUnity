...
void realtimeshow(int typee, int ongoing, double now, double best, double accu);
...
int main(void)
{
	...
	for (i = 0; i < 5; i++)				//5개만 놓는다.
	{
		r = rand() % 30;    //랜덤하게 놓는다.
		letter = 0;
		realtimeshow(letter, ongoing, nowspeed, bestspeed, accuracy);
...
void realtimeshow(int typee, int ongoing, double now, double best, double accu)
{
	...
	printf("진행도 : %-5d 현재타수 : %-5.0f 최고타수 : %-5.0f 정확도 : %-5.0f\n", ongoing, now, best, accu);

	printf("%s\n", moongoo);  //문구를 놓는다.

	while (a < typee)
		printf("%c", typing[a++]); //나의 타이핑을 놓는다.
