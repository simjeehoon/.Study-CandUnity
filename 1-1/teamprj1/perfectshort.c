...
int main(void)
{
	int okletter = 0, totalletter = 0, speedletter = 0; //여태까지 맞은 총 문자수, 입력한 총 문자수, 현재문장의 맞은 문자수 
	...
	while(i < 5)  //5번 출력합니다.
	{
		...
		speedletter = 0;  //현재 문장의 맞은 타수를 초기화합니다.
			...
			else if (alpha == 8 || alpha == 127) // 백스페이스
			{
				if (pointing != 0)
				{
					...
					if (payt == 0)
						...
						nowspeed = (60 * speedletter) / payt; // 분당 타수를 저장합니다.
					///////////////
...
//꼭 삭제하시오	  printf(" OK : %d  ||  TOTAL : %d  ||  Pointing : %d || speedletter : %d || this 길이 : %d\n", okletter, totalletter, pointing, speedletter, strlen(moongoo[r]));  
		...
		if (pointing < strlen(moongoo[r])) //주어진 문구 길이보다 많은 문자를 입력했을 때
			totalletter += strlen(moongoo[r]) - pointing;  //넘은 문자수를 입력한 총 문자수에 더합니다.

		accuracy = ((double)okletter / totalletter) * 100;  //정확도를 재측정합니다.
		ongoing += 20; //진행도 증가
